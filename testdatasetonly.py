...
MISSING_PERCENTAGES = [30, 50, 70, 80, 90]
DELTA_PERCENTAGE = 10
...
def make_nested_primary_masks():
    rng = np.random.default_rng(
        TEST_SEED
    )

    removal_order = rng.permutation(
        N_GY
    )
...
def save_percentage_dataset(
    ...
    gather_metadata = {
        ...
        "mask_policy": (
            "First remove the requested primary percentage "
            "from all 201 traces. Then remove DELTA_PERCENTAGE "
            "of the traces that survive the primary mask. "
            "The saved mask_delta file is the final combined mask."
        ),
...
def main():
    ...
    print(
        "\nFinished creating all five test-only datasets.",
        flush=True,
    )
