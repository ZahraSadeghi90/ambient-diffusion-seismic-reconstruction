...
REGULAR_MASK_CONFIGS = {
    50: {"period": 2, "pattern": "RK"},
    67: {"period": 3, "pattern": "RRK"},
    75: {"period": 4, "pattern": "RRRK"},
    80: {"period": 5, "pattern": "RRRRK"},
}
MISSING_PERCENTAGES = list(REGULAR_MASK_CONFIGS)
DELTA_PERCENTAGE = 10
...
def make_regular_primary_masks():
    masks = {}

    for missing_percentage, config in REGULAR_MASK_CONFIGS.items():
        ...
        observed = int(
            np.sum(column_mask > 0.5)
        )
        missing = N_GY - observed

        print(
            f"Regular primary mask p={missing_percentage}% | "
            f"pattern={config['pattern']} | "
            f"period={period} | "
            f"missing={missing} | observed={observed} | "
            f"actual missing={100.0 * missing / N_GY:.3f}%",
            flush=True,
        )
...
def save_percentage_dataset(
    ...
    out_root = (
        DATA_ROOT
        / (
            f"gx_fixed_p{missing_percentage}_"
            f"d{DELTA_PERCENTAGE}_testreg"
        )
    )
    ...
    gather_metadata = {
        ...
        "mask_policy": (
            "The primary mask is deterministic and periodic: "
            "R denotes a removed trace and K denotes a kept trace. "
            "Then DELTA_PERCENTAGE of the traces surviving the "
            "primary regular mask are removed randomly. The saved "
...
def main():
    ...
    print(
        "\nFinished creating all four regular test-only datasets.",
        flush=True,
    )
